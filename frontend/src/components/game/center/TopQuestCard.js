...
const TopQuestCard = ({ turnStart, choicesReveal, questCycle, onePlayer, choice, leftRound }) => {
  ...
  const flipQuest = <button type="button" className="bg-orange-500 font-bold p-2 animate-pulse" onClick={ onePlayer && !choice ? choicesReveal : turnStart }>{`${onePlayer && !choice ? 'Leave?' : '?QUEST?'}`}</button>
